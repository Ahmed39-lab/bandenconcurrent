...
export default function Header() {
  ...
  const megaMenuColumns = [
    ["Car tires", "Winter tires"],
    ["All-season tires", "Tire brands"],
    ["Run-flat tires", "TipAPK"],
    ["Assembly"],
  ];
  ...
  return (
    ...
    `}
                  >
                    ...
                    key={item.slug}
                    slug={item.slug}
                    openIndex={openIndex}           // current openIndex
                    setOpenIndex={setOpenIndex}     // state setter
                     />
                  </div>
                )}
              </li>
     ...
     <Link href={`${baseUrl}`}> 
      ...
      />
        ...
        {/* Mobile Menu */}
        {mobileMenuOpen && (
          <nav className="md:hidden mt-4 bg-white shadow-lg border-t border-gray-200">
            <ul>
              {menuItems.map((item) => (
